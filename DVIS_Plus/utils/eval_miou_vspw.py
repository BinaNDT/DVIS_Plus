...
class Evaluator(object):
    ...
    def Pixel_Accuracy(self):
        try:
            ...
            print("Error in Pixel_Accuracy: Division by zero")
            return 0
    ...
    def Pixel_Accuracy_Class(self):
        try:
            ...
            print("Error in Pixel_Accuracy_Class: Division by zero")
            return 0
    ...
    def Mean_Intersection_over_Union(self):
        try:
            ...
            print("Error in Mean_Intersection_over_Union: Division by zero")
            return 0
    ...
    def Frequency_Weighted_Intersection_over_Union(self):
        try:
            ...
            print("Error in Frequency_Weighted_Intersection_over_Union: Division by zero")
            return 0
    ...
    def _generate_matrix(self, gt_image, pre_image):
        try:
            ...
            print(f"Error in _generate_matrix: {e}")
            return np.zeros((self.num_class, self.num_class))

    def add_batch(self, gt_image, pre_image):
        try:
            assert gt_image.shape == pre_image.shape, "Shape mismatch between gt_image and pre_image"
            self.confusion_matrix += self._generate_matrix(gt_image, pre_image)
        except AssertionError as e:
            print(f"Error in add_batch: {e}")
        except Exception as e:
            print(f"Unexpected error in add_batch: {e}")
...
DIR=sys.argv[1]
split = 'test.txt' # Changed to test.txt

try:
    with open(os.path.join(DIR, split), 'r') as f:
        lines = f.readlines()
        videolist = [line.strip() for line in lines]
